...
def runBlastThread(blastParams : BlastParams, jobId, queryResultId, randomState):
    try:
        ...
        # Run BLAST
        blastArgs = {k: v for k, v in blastParams.dict().items() if v is not None and k != 'seq_length'}
        ...
        percent_identity = [(id_val / align_len) * 100 for id_val, align_len in zip(identity, align_length)]
        ...
        results = {
            "query_results" : query_resultJson,
...
def extract_protein_info(hit_def):
    protein_description = re.split(r'\s?\[', hit_def)[0].strip()  # Protein name and description
