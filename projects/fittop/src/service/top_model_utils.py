...
def loadESMseqs(seqs_df, bucket_name, model_path):
    ...
    for i in range(N_BATCHES):
        ...
        this_batch_df.insert(1, "fitness", seqs_df.fitness[start_seq:end_seq].reset_index(drop=True))

        this_df = pd.concat([this_df.reset_index(drop=True), this_batch_df.reset_index(drop=True)]).reset_index(drop=True)
    ...
    assert not this_df[feature_cols].isna().any().any(), "Still found NaNs after dropping!"
    return this_df
